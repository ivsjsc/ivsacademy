  ...
  return (
    <div id="ivs-react-fab-full" style={{position:'fixed',right:16,bottom:16,zIndex:9999}}>
      <div style={{display:'flex',flexDirection:'column',gap:12,alignItems:'flex-end'}}>
        ...
        <button id="ivs-react-open-assistant" onClick={()=>{ try{ (window as any).mountIVSReactComponents?.mountAssistant('#assistant-placeholder') }catch(e){} }} style={{width:56,height:56,borderRadius:999,background:'#0b5fff',color:'#fff',fontWeight:700}}>Chat</button>
      </div>
    </div>
  )
}
